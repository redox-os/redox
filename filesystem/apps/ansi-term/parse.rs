        ...
        for c in self.iter {
            match c {
                ...
                _ => match c {
                    ...
                    'C' if !params.is_empty() => return OutputEscapeCode::CursorRight(params[0]),
                    ...
                    'F' if !params.is_empty() => return OutputEscapeCode::PreviousLine(params[0]),
                    'F' => return OutputEscapeCode::PreviousLine(1),
                    // Goto
                    'G' if !params.is_empty() => return params[0],
                    'G' => return 1,
                    'H' | 'f' if params.len() >= 2 => return OutputEscapeCode::Goto(params[0], params[1]),
                    'H' | 'f' if params.len() == 1 => return OutputEscapeCode::Goto(params[0], 1),
                    'H' | 'f' => return OutputEscapeCode::GotoStart,
                    // Erase
                    'J' if params.is_empty() || params[0] == 0 => return OutputEscapeCode::EraseAfter,
                    'J' if params[0] == 1 => return OutputEscapeCode::EraseBefore,
                    'J' if params[0] == 2 => return OutputEscapeCode::EraseAll,
                    // Erase in line
                    'K' if params.is_empty() || params[0] == 0 => return OutputEscapeCode::EraseLineAfter,
