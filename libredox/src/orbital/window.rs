    ...
    pub fn new(x: isize, y: isize, w: usize, h: usize, title: &str) -> Option<Box<Self>> {
        ...
        unsafe {
            ...
            if ret.ptr as usize > 0 {
                // TODO: This is evil figure out a better way to do it
                let mut q = Session::commands();
                let mut guard = q.queue.lock();
                let mut session_cmds = guard.deref_mut();
                session_cmds.push(SessionCommand::AddWindow(ret.ptr));
                debug("add window\n");
            }
        }
    ...
    pub fn redraw(&mut self) {
        ...
        unsafe {
            ...
            */
             ...
             let mut guard = q.queue.lock();
             let mut session_cmds = guard.deref_mut();
             session_cmds.push(SessionCommand::Redraw);//.redraw = true;
        }
    }
    ...
    fn drop(&mut self) {
        unsafe {
            if self.ptr as usize > 0 {
                ...
                let mut guard = q.queue.lock();
                let mut session_cmds = guard.deref_mut();
                session_cmds.push(SessionCommand::RemoveWindow(self.ptr));
                debug("remove window\n");
            }
        }
    }
