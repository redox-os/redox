...
impl BMP {
    pub unsafe fn new(file_data: u32) -> BMP {
        let data;
        let size;
        if file_data > 0
        ...
        {
            let file_size = *((file_data + 0x2) as *const u32);
            let offset = *((file_data + 0xA) as *const u32);
            let width = *((file_data + 0x12) as *const u32);
            let height = *((file_data + 0x16) as *const u32);
            let depth = *((file_data + 0x1C) as *const u16) as u32;
    ...
    pub unsafe fn pixel(&self, point: Point) -> Color {
        ...
        {
            return *((self.data + (point.y as u32*self.size.width + point.x as u32)*4) as *const Color);
        }else{
