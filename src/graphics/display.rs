...
impl Display {
    ...
    unsafe fn fast_pixel(pixel_ptr: usize, color: Color){
        if color.a == 255 {
            *(pixel_ptr as *mut u8) = color.b;
            *((pixel_ptr + 1) as *mut u8) = color.g;
            *((pixel_ptr + 2) as *mut u8) = color.r;
        }else{
            ...
            let o_b = *(pixel_ptr as *const u8) as usize;
            let o_g = *((pixel_ptr + 1) as *const u8) as usize;
            let o_r = *((pixel_ptr + 2) as *const u8) as usize;
            let o_a = 255 - a;

            *(pixel_ptr as *mut u8) = ((o_b * o_a + b * a)/255) as u8;
            *((pixel_ptr + 1) as *mut u8) = ((o_g * o_a + g * a)/255) as u8;
            *((pixel_ptr + 2) as *mut u8) = ((o_r * o_a + r * a)/255) as u8;
        }
    }
        ...
        unsafe{
            if color.a > 0 {
                if point.x >= 0 && point.x < self.size.width as i32 && point.y >= 0 && point.y < self.size.height as i32 {
                    let pixel_ptr: usize = self.offscreen + point.y as usize * self.bytesperrow + point.x as usize * self.bytesperpixel;
    ...
    pub fn rect(&self, point: Point, size: Size, color: Color){
        if color.a > 0 {
            ...
            for y in start_y..end_y {
                unsafe{
                    let row_ptr: usize = self.offscreen + y * self.bytesperrow;
    ...
    pub fn clear(&self, color:Color){
        unsafe {
            let mut i = 0;
            while i < self.size.height as usize * self.bytesperrow {
                *((self.offscreen + i) as *mut u8) = color.b;
                *((self.offscreen + i + 1) as *mut u8) = color.g;
                *((self.offscreen + i + 2) as *mut u8) = color.r;
                i += 3;
            }
        }
    }
    ...
    pub unsafe fn c_text(&self, point: Point, c_text: *const u8, color: Color){
        let mut cursor = Point::new(point.x, point.y);
        for i in 0..(self.size.width as usize - point.x as usize)/8 {
    ...
    pub fn copy(&self){
        unsafe{
            let mut i = 0;
            while i < self.size.height as usize * self.bytesperrow {
                let data: u64 = *((self.offscreen + i) as *const u64);
                *((self.onscreen + i) as *mut u64) = data;
                i += 8;
            }
        }
    }
