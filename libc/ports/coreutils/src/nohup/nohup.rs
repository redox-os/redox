...
use libc::funcs::posix88::uniredox::{dup2, execvp, isatty};
use libc::consts::os::posix01::SIG_IGN;
use libc::consts::os::posix88::SIGHUP;
use redox::env;
use redox::ffi::CString;
use redox::fs::{File, OpenOptions};
use redox::io::{Error, Write};
use redox::os::unix::prelude::*;
use redox::path::{Path, PathBuf};
...
unsafe fn _vprocmgr_detach_from_console(_: u32) -> *const libc::c_int { redox::ptr::null() }
...
pub fn uumain(args: Vec<String>) -> i32 {
    ...
    if unsafe { _vprocmgr_detach_from_console(0) } != redox::ptr::null() { crash!(2, "Cannot detach from console")};

    let cstrs: Vec<CString> = matches.free.iter().map(|x| CString::new(x.as_bytes()).unwrap()).collect();
    let mut args: Vec<*const c_char> = cstrs.iter().map(|s| s.as_ptr()).collect();
    args.push(redox::ptr::null());
    unsafe { execvp(args[0], args.as_mut_ptr())}
}

fn replace_fds() {
    let replace_stdin = unsafe { isatty(libc::redoxIN_FILENO) == 1 };
    let replace_stdout = unsafe { isatty(libc::redoxOUT_FILENO) == 1 };
    let replace_stderr = unsafe { isatty(libc::redoxERR_FILENO) == 1 };
