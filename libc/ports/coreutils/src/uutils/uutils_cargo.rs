...
use redox::env;
use redox::collections::hash_map::HashMap;
use redox::path::Path;
...
fn main() {
    ...
    match umap.get(binary_as_util) {
        Some(&uumain) => {
            redox::process::exit(uumain(args));
        }
        None => (),
    }
    ...
    if args.len() >= 2 {
        ...
        match umap.get(util) {
            Some(&uumain) => {
                redox::process::exit(uumain(args.clone()));
            }
            None => {
                if &args[0][..] == "--help" {
                    ...
                    if args.len() >= 2 {
                        ...
                        match umap.get(util) {
                            Some(&uumain) => {
                                redox::process::exit(uumain(vec![util.to_string(), "--help".to_string()]));
                            }
                            None => {
